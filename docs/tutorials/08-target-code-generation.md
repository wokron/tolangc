...
- `$t0 - $t7, $t8 - $t9`：临时寄存器，仅在当前函数中有效，调用其他函数时不必保存。
- `$s0 - $s7`（saved）：全局寄存器，这些寄存器往往作用域范围较大，会在程序的不同位置使用，因此在函数调用时往往需要进行保存。
...
生成的 MIPS 代码如下，可以看到每次计算都从栈中加载变量，并将结果存入对应变量。这里需要注意，对于一些比较复杂的表达式，可能需要更多的寄存器，因此还是需要避免冲突。
...
不考虑优化的情况下，大家可以不用考虑复杂的寄存器分配方法，只需要能够生成正确的 MIPS 代码，后续在优化部分可以采用课上讲到的图着色或者是其他方式进行寄存器分配。
...
生成函数调用的难点在于参数的处理，函数调用主要涉及以下几个步骤。
...
2. 保存调用现场。一般来说，调用者只需要保存调用时使用的全局寄存器，而被调用者需要保存调用者的 `$fp` 以及自己的 `$ra`。
3. 函数跳转。通过 `jal` 或 `jalr` 指令跳转到被调用的函数，此时被调用者需要接收参数，并更新 `$sp` 从而创建自己的栈帧。
4. 返回。被调用函数执行结束后，将返回值（如果有的话）保存到 `$v0`，恢复 `$sp` 与 `$ra`，返回调用者。
...
当然这里只是给出了一种函数调用的实现方式，也可以将一些寄存器的保存交给调用者，大家自行设计，合理即可。
